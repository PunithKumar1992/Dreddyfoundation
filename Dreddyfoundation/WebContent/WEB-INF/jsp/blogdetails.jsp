<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
    pageEncoding="ISO-8859-1"%>
<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
<html >
    <head>

    <meta charset="utf-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Dayanadfoundation</title>
<link rel="shortcut icon" type="image/png" href="images/home/LOGO.png"> 

   
    <link href="assets/css/bootstrap.min.css" rel="stylesheet">
   
    <link href="assets/css/icofont.css" rel="stylesheet">
  
    <link href="assets/css/magnific-popup.css" rel="stylesheet">
  
    <link href="assets/css/jquery.lineProgressbar.css" rel="stylesheet">
 
    <link href="assets/css/owl.carousel.css" rel="stylesheet">
  
    <link href="assets/css/jquery-ui.min.css" rel="stylesheet">
  
    <link href="assets/css/slicknav.min.css" rel="stylesheet">
    
    <link href="assets/css/animate.css" rel="stylesheet">
   
    <link href="assets/css/style.css" rel="stylesheet">
 
    <link href="assets/css/responsive.css" rel="stylesheet">

</head>

<body>

<div class="navbar-section">
        <div class="container">
            <div class="row">
                <div class="col-md-2 col-sm-2">
                    <a href="index" class="logo">
                        <img src="images/home/LOGO.png" alt="logo image">
                    </a>
                </div>
  <div class="col-md-10 col-sm-10">
                    <div class="row">
                        <div class="col-md-8 col-md-offset-4 col-sm-10">
                            <div class="support-bar-wrapper ">
                              
                               <div class="support-content">
                                   <a href="" class="support-email"> <i class="icofont icofont-phone"></i>9943607961<span class="separetor">&#10072;</span></a>
                                    <a href="#" class="support-number"> <i class="icofont icofont-ui-dial-phone"></i> +123 456 789 000 <span class="separetor">&#10072;</span></a>

                                    </div> 
                                </div>
                            </div> 
                        </div> 
                    <div class="col-md-9 text-right">
                            <ul id="main-menu" class="main-menu">
                                <li><a href="index">Home</a></li>
                                <li><a href="about">ABOUT</a></li>
                                <li><a href="Blog">Blog<i class="icofont icofont-thin-down"></i></a>
                                    <ul class="sub-menu">
                                        <li><a href="Blog">Blog</a></li>
                                        <!-- <li><a href="blogdetails">BlogDetails</a></li> -->
                                    </ul>
                                </li>
                                <li><a href="education">Education<i class="icofont icofont-thin-down"></i></a>
                                    <ul class="sub-menu">
                                        
                                    </ul>
                                </li>
                                <li><a href="gallery">Gallery<i class="icofont icofont-thin-down"></i></a>
                                    <ul class="sub-menu">
                                        </ul>
                                </li>
                                <li><a href="volunteer">Volunteer<i class="icofont icofont-thin-down"></i></a>
                                    <ul class="sub-menu">
                                     
                                    </ul>
                                </li>
                                <li><a href="contact">CONTACT</a></li>
                            </ul>
                        </div>
                        <div class="col-md-3 col-sm-1 text-right">
                            <div class="tools">
                              <a href="https://www.google.com" id="search-icon" name="search_text" ><i class="icofont icofont-search"  style="color:#000000"></i></a>
                               <span class="search-icon">
                      
                    </span>
                            </div>
                           
                            <div class="support-social ">
                                       <a href="https://www.facebook.com/BTdayanandareddy/"><i class="icofont icofont-social-facebook"></i></a>
                                        <a href="https://twitter.com/btdayanandreddy"><i class="icofont icofont-social-twitter"></i></a>
                                        <a href="https://www.youtube.com/watch?v=bs8hdfWTud4&feature=youtu.be"><i class="icofont icofont-social-google-plus"></i></a>
                                        <a href="https://www.instagram.com/dayanandareddyofficial/"><i class="icofont icofont-social-instagram"></i></a>
                                    </div>
                               
                                    
                           
                        </div>
                    </div>
                </div>
            </div>
        </div>
  
    
 
    <div class="overlay-bg"></div>

<section class="breadcumb-section breadcumb-bg">
    <div class="container">
        <div class="row">
            <div class="col-md-12">
                <h1>Blog Detail</h1>
               <p><a href="index">Home</a>/
                Blog Detail</p>
            </div>
        </div>
    </div>
</section>

  
    <div class="single-causes">
        <div class="container">
            <div class="row" >
             <div class="section-title">
                        <h2 Align="center">BLOG<span>Details</span></h2>
                        
                    </div>
                <div class="col-md-12">
               
                    <div class="single-cause-thumb">
                    
                        <img src="images/1170x500.jpg" alt="">
                    </div>
                    <div class="box-shadow-bottom"></div>
                    <div class="prog-item">
                       
                    </div>
                    </div>
                    <div class="cause-amount-collected">
                        <div class="row">
                        
                    </div>
                    </div>
                
                <div class="single-cause-content">
                    <p>Dayananda Reddy was born to B.N Thimma Reddy and Gowramma in 1963 at Anekal, Bengaluru urban district and finished his education in government school from a very young age he used to believe and still believe in “knowledge is wealth” and showed his ability to lead his organizational capability right from the tender age</p>
                    <div class="row">
                        <div class="col-md-6">
                            <p>His first focus was on education and employment issues of the youth, as he found a huge disparity in the education system of rural and urban students </p>
                            <p>He also noted the following statistics of employment and employability skills of Indian youth, so this leads to Dayananda foundation in 2010. </p>
                        </div>
                        <div class="col-md-6">
                            <ul class="single-cause-ul">
                                <li><i class="icofont icofont-verification-check"></i> Only he changed the Education system..</li>
                                <li><i class="icofont icofont-verification-check"></i>Only he provide the UNDEREMPLOYMENT for students..</li>
                                <li><i class="icofont icofont-verification-check"></i> He distributed his contribution for protecting enviroment </li>
                                <li><i class="icofont icofont-verification-check"></i> Only he adopted the 7500 underprivileged childerns</li>
                            </ul>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
    
    <div class="recent-causes">
        <div class="container">
            <div class="row">
                <div class="col-md-8 col-md-offset-2 text-center">
                    <div class="section-title">
                        <h2>Read<span>More</span></h2>
                        <span class="title-separetor">
                                <img src="images/health-care.png" alt="separetor image">
                            </span>
                    </div>
                </div>
            </div>
            <div class="row">
                <div class="col-md-4">
                    <div class="single-causes-item single-page">
                        <div class="cause-thumb">
                            <img src="images/370x2601.jpg" alt="case image">
                            <div class="box-shadow-bottom"></div>
                            <div class="prog-item">
                                
                            </div>
                        </div>
                       
                        <div class="cause-right">
                            <div class="cause-right-top">
                                <h4>Educational<span class="color">Events</span></h4>
                                <p>Thank you all for being part of a wonderful cause today, Dayananda foundation celebrates its 6th anniversary today reaching a new milestone of 7500 + adopted students.
                                 </p>
                            </div>
                            <div class="cause-right-bottom">
                                
                                <div class="cause-right-social-icon text-right">
                                   
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="single-causes-item single-page">
                        <div class="cause-thumb">
                            <img src="images/370x2602.jpg" alt="case image">
                            <div class="box-shadow-bottom"></div>
                            <div class="prog-item">
                                
                            </div>
                        </div>
                       
                        <div class="cause-right">
                            <div class="cause-right-top">
                                <h4>Educational <span class="color">events</span></h4>
                                <p>Responding to the needs of children and women in India Dayananda Foundation is a registered non - government organization, which works purely on a non proﬁt basis., </p>
                            </div>
                            <div class="cause-right-bottom">
                                
                                <div class="cause-right-social-icon text-right">
                                   
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="single-causes-item single-page">
                        <div class="cause-thumb">
                            <img src="images/370x2603.jpg" alt="case image">
                            <div class="box-shadow-bottom"></div>
                            <div class="prog-item">
                               
                            </div>
                        </div>
                        
                        <div class="cause-right">
                            <div class="cause-right-top">
                                <h4>Educations<span class="color">Events</span></h4>
                                <p>Adopted students of DayanandaFoundation gathered for Mr. Dayananda’s birthday celebrations in 2013, it has proved fruitful by athletics and players getting international medals.
</p>
                            </div>
                            <div class="cause-right-bottom">
                              
                                <div class="cause-right-social-icon text-right">
                                   
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
    
       <div class="upcoming-event-section">
        <div class="container">
            <div class="row">
                <div class="col-md-8 col-md-offset-2 text-center">
                    <div class="section-title">
                        <h2>Blog <span>Event</span></h2>
                        <span class="title-separetor">
	                        <img src="images/health-care.png" alt="separetor image">
	                    </span>
                    </div>
                </div>
            </div>
                <div class="row">
                   <div class="upcoming-event-wrapper">
                       <div class="single-event-list-box">
                           <div class="row">
                               <div class="col-md-5">
                                   <div class="event-list-post-thumb list-post-thumb-1">
                                    
                                   </div>
                               </div>
                               <div class="col-md-7">
                                   <div class="post-inner">
                                    <a href="#">
                                         <h4>Blood <span class="color">Donation Camp</span></h4>
                                    </a>
                                    <div class="post-meta">
                                        <p class="time"><i class="icofont icofont-wall-clock"></i> 9:30 AM</p>
                                        <a href="#" class="location"><i class="icofont icofont-location-pin"></i> spriya</a>
                                        <a href="#" class="comments"><i class="icofont icofont-speech-comments"></i> 7</a>
                                        <a href="#" class="user"><i class="icofont icofont-user"></i> Admin</a>
                                    </div>
                                    <div class="port-body">
                                        <p>Dayananda Reddy has been instrumental in organizing
numerous Complete Medical Health Camps, Sponsored many free Cleft-Lip surgery camps,
conducted various Blood Donation Camps, he has conducted workshops on spreading
awareness on health &amp; hygiene for the underprivileged, poor and rural women in Karnataka
State and has provided monitory aid to under privileged people for their treatments in well
reputed hospitals. </p>
                                      
                                    </div>
                                </div>
                               </div>
                           </div>
                       </div>
                       
                       <div class="single-event-list-box">
                           <div class="row">
                               <div class="col-md-5">
                                   <div class="event-list-post-thumb list-post-thumb-2">
                                       
                                   </div>
                               </div>
                               <div class="col-md-7">
                                   <div class="post-inner">
                                    <a href="#">
                                          <h4>Environmental<span class="color">Consern Camp</span></h4>
                                    </a>
                                    <div class="post-meta">
                                        <p class="time"><i class="icofont icofont-wall-clock"></i> 9:30 AM</p>
                                        <a href="#" class="location"><i class="icofont icofont-location-pin"></i>priya1</a>
                                        <a href="#" class="comments"><i class="icofont icofont-speech-comments"></i> 7</a>
                                        <a href="#" class="user"><i class="icofont icofont-user"></i> Admin</a>
                                    </div>
                                    <div class="port-body">
                                        <p>Every student of Dayananda Foundation
must plant a tree every year and nourish it.
Till date the students have planted a total of
50,000 + saplings to keep Bangalore green.</p>
                                   
                                    </div>
                                </div>
                               </div>
                           </div>
                       </div>
                       
                       <div class="single-event-list-box">
                           <div class="row">
                               <div class="col-md-5">
                                   <div class="event-list-post-thumb list-post-thumb-3">
                                       
                                   </div>
                               </div>
                               <div class="col-md-7">
                                   <div class="post-inner">
                                    <a href="#">
                                        <h4>Awareness for<span class="color"> saving water</span></h4>
                                    </a>
                                    <div class="post-meta">
                                        <p class="time"><i class="icofont icofont-wall-clock"></i> 9:30 AM</p>
                                        <a href="#" class="location"><i class="icofont icofont-location-pin"></i>priya</a>
                                        <a href="#" class="comments"><i class="icofont icofont-speech-comments"></i> 7</a>
                                        <a href="#" class="user"><i class="icofont icofont-user"></i> Admin</a>
                                    </div>
                                    <div class="port-body">
                                        <p>providing  drinking water to remote  villages  through Bore wells.(DF) has made a remarkable contribution towards community development since its inception. he is been proactively engaged in
numerous activities</p>
                                       
                                    </div>
                                </div>
                               </div>
                           </div>
                       </div>
                       
                       <div class="single-event-list-box">
                           <div class="row">
                               <div class="col-md-5">
                                   <div class="event-list-post-thumb list-post-thumb-4">
                                      
                                   </div>
                               </div>
                               <div class="col-md-7">
                                   <div class="post-inner">
                                    <a href="#">
                                    <h4>Support for <span class="color">Quality Education:</span></h4>
                                    </a>
                                    <div class="post-meta">
                                        <p class="time"><i class="icofont icofont-wall-clock"></i> 9:30 AM</p>
                                        <a href="#" class="location"><i class="icofont icofont-location-pin"></i>priya</a>
                                        <a href="#" class="comments"><i class="icofont icofont-speech-comments"></i> 7</a>
                                        <a href="#" class="user"><i class="icofont icofont-user"></i> Admin</a>
                                    </div>
                                    <div class="port-body">
                                       <p> The idea of this project is to provide 3 years of high school education i.e. 8th, 9th, 10th and
after which technical education based on the requirement of more than 500 affiliated
industries to ensure employment. This program is helping them to get free education and
employment which leads in strengthening the economic growth of an individual, their family
and the country make them self-sustainable.</p>
                                    </div>
                                </div>
                               </div>
                           </div>
                       </div>
                    </div>
                </div>
           
        </div>
    </div>
    
    
    
    <div class="newsletter">
            <div class="container">
                <div class="row">
                    <div class="col-md-12">
                        <div class="news-letter-wrapper">
                            <div class="row">
                                <div class="col-md-5">
                                    <span class="sub-title">Subscribe</span>
                                    <h4>to our newsletter</h4>
                                </div>
                                <div class="col-md-7">
                                    <form action="">
                                        <div class="form-element">
                                            <input type="email" name="email" placeholder="Enter your email address....">
                                            <span class="has-icon"><input type="submit" value="submit"></span>
                                        </div>
                                    </form>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
   
   <footer class="footer-section">
        <div class="container">
            <div class="row">
                <div class="col-md-3 col-sm-6">
                    <div class="widget-one-wrapper">
                        <div class="footer-logo">
                            <img src="images/home/LOGO.png" alt="footer logo">
                        </div>
                        <div class="widget-one-body">
                            <p>With the new day comes new strength and new thoughts</p>
                            <a href="#" class="boxed-btn">Read More <i class="icofont icofont-curved-double-right"></i></a>
                            <div class="footer-social-links">
                                <a href="https://www.facebook.com/BTdayanandareddy/"><i class="icofont icofont-social-facebook"></i></a>
                                        <a href="https://twitter.com/btdayanandreddy"><i class="icofont icofont-social-twitter"></i></a>
                                        <a href="https://www.youtube.com/watch?v=bs8hdfWTud4&feature=youtu.be"><i class="icofont icofont-social-google-plus"></i></a>
                                        <a href="https://www.instagram.com/dayanandareddyofficial/"><i class="icofont icofont-social-instagram"></i></a>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="col-md-3 col-sm-6">
                    <div class="widget-two-wrapper">
                        <div class="widget-title">
                            <h4>Contact us</h4>
                            <span class="title-separetor">
                                 <img src="images/health-care.png" alt="separetor image">
                             </span>
                        </div>
                        <div class="widget-two-body">
                            <p><i class="icofont icofont-phone"></i> + 123 456 789 000</p>
                            <p><a href="mailTo:example@gmail.com"><i class="icofont icofont-email"></i>foundation@gmail.com</a></p>
                            <p><a href="#"><i class="icofont icofont-web"></i> foundation.com</a></p>
                            <p><i class="icofont icofont-location-pin"></i>Rajajinagar,d -block,mkk road</p>
                        </div>
                    </div>
                </div>
                <div class="col-md-3 col-sm-7">
                    <div class="widget-three-wrapper">
                        <div class="widget-title">
                            <h4>Header Links</h4>
                            <span class="title-separetor">
                                 <img src="images/health-care.png" alt="separetor image">
                             </span>
                        </div>
                        <div class="widget-three-body">
                            <ul>
                                <li><a href="event-list"><i class="icofont icofont-hand-right"></i> Home</a></li>
                                <li><a href="single-cause"><i class="icofont icofont-hand-right"></i>about us</a></li>
                                <li><a href="donate"><i class="icofont icofont-hand-right"></i>education</a></li>
                                <li><a href="contact"><i class="icofont icofont-hand-right"></i>gallery</a></li>
                                <li><a href="sign-up"><i class="icofont icofont-hand-right"></i>volunteer</a></li>
                                <li><a href="#"><i class="icofont icofont-hand-right"></i>Contact</a></li>
                                
                            </ul>
                        </div>
                    </div>
                    
                    
                </div>
                    <div class="col-md-3 col-sm-5">
                    <div class="widget-four-wrapper">
                        <div class="widget-title">
                        <h4>Contact Form</h4>
                         <span class="title-separetor">
                                 <img src="images/health-care.png" alt="separetor image">
                             </span>
                            <div class="widget-four-bodyyy">
<form>
    
							<input type="text" name="name"  placeholder="Full Name">
							<br><br>
							<input type="text" name="email" placeholder="Email Address" >
							<br><br>
							<textarea name="message" placeholder="Your Message"></textarea>
							  <a href="#" class="boxed-btn">submit</a>
						</form> 
                    </div>   
                    </div>     
            </div>
        </div>
    </footer>
   
   <div class="copyright-area">
        <div class="container">
            <div class="row">
                <div class="col-md-6 col-sm-6">
                  <p> © 2018 Dayananda Reddy. All Rights Reserved </p> 
                </div>
                <div class="col-md-6 col-sm-6 ">
                    <div class="footer-menu">
                        <a href="http://www.appfonehub.com/">Designed by Appfone Hub Pvt Ltd.</a><span>,</span>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <div class="scroll-to-top">
        <i class="icofont icofont-rounded-up"></i>
    </div>
    

    <script src="assets/js/jquery.js"></script>
   
    <script src="assets/js/isotope.pkgd.js"></script>
 
    <script src="assets/js/jquery.magnific-popup.js"></script>
   
    <script src="assets/js/waypoints.min.js"></script>
  


    <script src="assets/js/jquery.counterup.min.js"></script>
   
    <script src="assets/js/jquery.countdown.js"></script>
 
    <script src="assets/js/owl.carousel.min.js"></script>
  
    <script src="assets/js/imagesloaded.pkgd.js"></script>
   
    <script src="assets/js/bootstrap.min.js"></script>
   
    <script src="assets/js/jquery.slicknav.min.js"></script>

    <script src="assets/js/jquery-ui.min.js"></script>
  
    
    <script src="assets/js/wow.min.js"></script>
   
    <script src="assets/js/main.js"></script>
</body>

</html>



